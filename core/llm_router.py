...
async def stream_answer(transcript: str, engine: str) -> None:
    ...
    # Get question type before processing
...
{transcript}"""
    ...
    if engine == "groq":
        ...
        try:
            ...
            print(f"[llm_router] ✓ Groq response complete", flush=True)
        ...
        try:
            ...
            print(f"[llm_router] ✓ Claude response complete", flush=True)
            broadcast({"type": "answer_done"})
    ...
    # Broadcast question type to frontend (after answer is done)
